<?php
/**
 * Dashboard Charts API v2 - Working Version
 * 
 * Provides Chart.js compatible data for supplier dashboard visualizations
 * Uses exact database schema fields as verified via knowledge base
 * 
...
try {
    ...
    switch ($chartType) {
        case 'orders_trend':
            // 30-day orders trend - using exact schema with flexible deleted_at
        ...
        case 'top_products':
            // Top 10 products by units shipped - using exact schema with flexible deleted_at
            $transfersCondition = getNotDeletedCondition('transfers', 't');
        ...
        case 'outlet_performance':
            // Orders by outlet - using exact schema with flexible deleted_at
            $transfersCondition = getNotDeletedCondition('transfers', 't');
        ...
        case 'warranty_trends':
            // Weekly warranty claims - using exact schema with flexible deleted_at
            $productsCondition = getNotDeletedCondition('vend_products', 'p');
            $query = "
                SELECT 
        ...
        case 'revenue_analysis':
            // Monthly revenue for last 12 months - using exact schema with flexible deleted_at
            $transfersCondition = getNotDeletedCondition('transfers', 't');
            $query = "
                SELECT 
